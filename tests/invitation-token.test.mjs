...
test("round-trips a numbered guest invitation", async () => {
  ...
  assert.deepEqual(await decodeInvitationToken(token), { people: 4, days: 2, side: "pooja" });
...
test("round-trips a whole-family invitation without a guest count", async () => {
  ...
  assert.deepEqual(await decodeInvitationToken(token), { people: null, days: 3, side: "meet" });
});
